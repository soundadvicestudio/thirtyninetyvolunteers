...
export async function GET(request: Request) {
  try {
    ...
    // A. Target date: CURRENT_DATE - 2 in the org timezone (DST-safe — see
    // QuickStats.tsx getUpcomingShowsThisMonth for the same pattern).
    // show_date is a bare date column, so once we have the correct
    // org-timezone calendar day, plain UTC date arithmetic on it is safe
