...
## 30BN_BRIEF_v1.md — Complete & Authoritative — v5.0
### Created: July 2026 | Last Updated: August 2026 — v5.0 (DOC.64: 033 migration applied; ADMIN.43 proxy.ts auditions Production fix; INVENTORY.1 complete — Migration 034 applied, feature_inventory flag, inventory_manager toggle on User Management, Sidebar TOOLTIP_ANCHOR_MAP refactor, HelpContent 16th section; §7 Phase AUDITIONS proxy inaccuracy corrected; §9 Migration 033+034 status blocks added; §11 INVENTORY.1 build summary added)
...
**Current phase:** Phase AUDITIONS complete (AUDITIONS.A–4b, Migration 032). Migration 033 (audition schema fixes) applied. Phase INVENTORY in progress — INVENTORY.A (audit) and INVENTORY.1 (Migration 034, flag infrastructure, sidebar, user management toggle, stub page, HelpContent) complete. INVENTORY.2–5 pending. Phase FORUMS confirmed as pre-launch build. Phase 17 (Launch) follows. Phase CAST planned post-launch.
...
- **`inventory_manager` toggle** (INVENTORY.1): on each Editor row only — grants or revokes full inventory write access (create/edit/deactivate items, manage categories and locations, create checkouts). Toggle absent on Super Admin, Owner Admin, Viewer, and Production rows (SA/OA always have full inventory access; Viewer/Production have no inventory access). Calls `toggleInventoryManager()` in `lib/actions/users.ts`. Logged to `audit_log` as `user.inventory_manager_change` (type defined in `types/audit.ts`). DB CHECK constraint enforces `inventory_manager = false` on `production` and `viewer` accounts. `app/crew/(app)/settings/users/page.tsx` query updated to fetch `inventory_manager` column (unplanned addition confirmed correct in INVENTORY.1 F3). SA/OA callers only.
...
**30BN-INVENTORY.1 ✓** See Phase INVENTORY section above for full build summary. 13 files. Commit c367288.
